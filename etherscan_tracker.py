...
def get_holders(contract_address):
    url = f"{ETHPLORER_API}/getTopTokenHolders/{contract_address}?apiKey={ETHPLORER_KEY}&limit=300"
    try:
        res = requests.get(url)
        data = res.json()
        return data.get("holders", [])
    except Exception as e:
        print(f"[HATA] Holder verisi alınamadı: {e}")
        return []
...
def get_wallet_history(address):
    ...
    try:
        ...
        print(f"[HATA] İşlem geçmişi alınamadı: {e}")
        return []
...
def check_balina_transfer():
    for coin, data in COINS.items():
        print(f"[{coin}] Coin taranıyor...")
        ...
        print(f"[{coin}] kontrolü tamamlandı.")

def main():
    print("ZERODAY balina takibi başlatıldı.")
    while True:
        check_balina_transfer()
        print("Tüm coinler kontrol edildi. 60 dakika bekleniyor...\n")
        time.sleep(3600)
